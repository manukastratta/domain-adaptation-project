...
def get_heatmap(CAM_map, img):
    ...
    breakpoint()
    CAM_map = np.resize(CAM_map, (img.shape[0], img.shape[1]))
    ...
    cmap = Colormap.get_cmap_rainbow()
    heatmap = cmap(np.uint8(255 * CAM_map))

    # cm = cm.get_cmap('gist_rainbow')
    # heatmap = cm(np.uint8(255 * CAM_map))
...
def main():
    ...
    parser.add_argument('--outdir', type=str, default='/data/yusun/manuka_nicole_teddi/test-time-training-project/ttt_imagenet_release/cam_results/')
    ...
    df = pd.read_csv('/data/yusun/manuka_nicole_teddi/test-time-training-project/data/CelebA/splits/setting2_test.csv')
    ...
    for img_id in args.img_ids:
        ...
        class_labels =torch.tensor([df[df['img_filename']=='000830.jpg'].iloc[0]['Black_Hair']]).byte()
        ...
        # outputs = net(norm_img)
        # _, predicted = outputs.max(1)
        # confidence = nn.functional.softmax(outputs, dim=1).squeeze()[class_labels].item()
