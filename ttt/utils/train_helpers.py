...
def build_model(args):
        ...
        if args.depth == 152:
                ...
                net = models.resnet50(norm_layer=norm_layer, pretrained=args.use_pretrained).cuda()
                expansion = 4
        elif args.depth == 18:
...
def prepare_train_data(args):
        ...
        traindir = os.path.join(args.dataroot, 'img_align_celeba')
        labeldir = os.path.join(args.train_labels)
        trset = RotateImageFolder(traindir, labeldir, tr_transforms, original=True, rotation=True,
...
def prepare_test_data(args, use_transforms=True, return_imgnames=False):
        ...
        if not hasattr(args, 'corruption') or args.corruption == 'original':
                        ...
                        validdir = os.path.join(args.dataroot, 'img_align_celeba')
                        labeldir = os.path.join(args.val_labels)
                        teset = RotateImageFolder(validdir, labeldir, te_transforms_local, original=True, rotation=False, rotation_transform=rotation_te_transforms, return_imgnames=return_imgnames)

        elif args.corruption in common_corruptions:
                        print('Test on %s level %d' %(args.corruption, args.level))
                        validdir = os.path.join(args.dataroot, 'img_align_celeba')
                        labeldir = os.path.join(args.val_labels)
                        teset = RotateImageFolder(validdir, labeldir, te_transforms_local, original=False, rotation=False, rotation_transform=rotation_te_transforms, return_imgnames=return_imgnames)
        
        elif args.corruption == 'video':
                        validdir = os.path.join(args.dataroot, 'img_align_celeba')
                        labeldir = os.path.join(args.val_labels)
                        teset = ImagePathFolder(validdir, labeldir, te_transforms_local)
        else:
        ...
        if args.batch_examples > 1: # to support multi batch in test for online mode
                teloader = torch.utils.data.DataLoader(teset, batch_size=args.batch_examples, shuffle=False, num_workers=args.workers, pin_memory=True)
        else:
                teloader = torch.utils.data.DataLoader(teset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True)
